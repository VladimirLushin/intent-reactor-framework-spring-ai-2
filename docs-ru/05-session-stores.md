...
Один `save()` движка выполняет две перезаписи файла: сначала заголовок/envelope сессии, затем пакетную дозапись новых событий (все — одной атомарной перезаписью под одной блокировкой, а не перезаписью на каждое событие). При падении процесса между этими двумя записями последний шаг диалога может не дописаться и будет отсутствовать при следующей загрузке. Сама сессия остаётся целой, события append-only, так что ничего не повреждается: разговор просто продолжится с того, что успело сохраниться.
